...
%  be 1. The type can be an integer (MSP_DOUBLE etc.) or a character string
%  for MATLAB type 'double'. If not present, then type is MSP_DOUBLE.

if nargin<4; blkdim = int32(1); end
if nargin<5;
    type = MSP_DOUBLE;
elseif ischar(type_in)
    type = mspType(type_in, true);
else
    type = type_in;
end
...
vals = coder.nullcopy(repmat(mspZero(type),nnzb*blkdim*blkdim, 1));
