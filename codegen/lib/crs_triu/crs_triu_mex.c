 ...
 * %#m2c options: codegenArgs=-args {crs_matrix} crs_triu1 -args {crs_matrix, int32(1)}  enableInline=1 withOMP=1
 *
 */
...
void crs_triu_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=5)
        mexErrMsgIdAndTxt("crs_triu:InputStructWrongFields",
    ...
    *(void **)&U.val = mxCalloc(1, sizeof(emxArray__common));    init_emxArray((emxArray__common*)U.val, 1);
    ...
    mxSetFieldByNumber((mxArray*)(plhs[0]), 0, 4, copy_scalar_to_mxArray(&U.ncols, mxINT32_CLASS));
...
void crs_triu1_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (!mxIsStruct(prhs[0]))
        mexErrMsgIdAndTxt("crs_triu1:WrongInputType",
            "Input argument A has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[0])!=5)
        mexErrMsgIdAndTxt("crs_triu1:InputStructWrongFields",
            "Input argument A has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
