...
if A.ncols~=B.nrows
    msg_error('crs_prodMatMat:WrongSizes', 'Number of columns of A must be equal to number of rows in B.');
end
...
C.col_ind = nullcopy(zeros(C.row_ptr(A.nrows+1)-1,1,'int32'));
...
C.val = nullcopy(zeros(C.row_ptr(A.nrows+1)-1,1, class(A.val)));
