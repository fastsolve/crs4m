 ...
 * %#m2c options: codegenArgs=-args {CuBlasHandle, CuStreamHandle}  enableInline=1
 *
 */
...
void cuBlasSetStream_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("cuBlasSetStream:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("cuBlasSetStream:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    cuBlasSetStream_initialize();

    cuBlasSetStream(&hdl, &strm, errCode, toplevel);

    cuBlasSetStream_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 2) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("cuBlasSetStream:TooManyOutputArguments","Too many output arguments for entry-point cuBlasSetStream.");
        /* Call the API function. */
        cuBlasSetStream_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("cuBlasSetStream:WrongNumberOfInputs","Incorrect number of input variables for entry-point cuBlasSetStream.");
