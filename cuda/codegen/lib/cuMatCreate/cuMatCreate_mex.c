 ...
 * %#m2c options: codegenArgs=-args {int32(0), int32(0), int32(0)} cuMatCreate_2args -args {int32(0), int32(0)}  enableInline=1
 *
 */
...
void cuMatCreate_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[2], mxLOGICAL_CLASS, 2, l_size); }
    ...
    cuMatCreate_initialize();

    cuMatCreate(m, n, type, &mat, errCode, toplevel);

    cuMatCreate_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 3) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("cuMatCreate:TooManyOutputArguments","Too many output arguments for entry-point cuMatCreate.");
        /* Call the API function. */
        cuMatCreate_api(prhs, (const mxArray**)outputs);
    }
    else if (nrhs == 2) {
        if (nlhs > 3)
            mexErrMsgIdAndTxt("cuMatCreate_2args:TooManyOutputArguments","Too many output arguments for entry-point cuMatCreate_2args.");
        /* Call the API function. */
        cuMatCreate_2args_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("cuMatCreate:WrongNumberOfInputs","Incorrect number of input variables for entry-point cuMatCreate.");
