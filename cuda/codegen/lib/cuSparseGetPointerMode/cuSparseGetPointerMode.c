#include "cuSparseGetPointerMode.h"
#include "mspack.h"
#include "m2c.h"
...
static void b_m2c_error(const emxArray_char_T *varargin_3)
{
  ...
  M2C_error("CUDA:RuntimeError",
            "cusparseGetPointerMode returned error code %s\n",
            &b_varargin_3->data[0]);
...
void cuSparseGetPointerMode(const struct0_T *hdl, int *mode, int *errCode,
  boolean_T *toplevel)
{
  cusparsePointerMode_t t_mode;
  boolean_T p;
  boolean_T b_p;
  int k;
  int exitg2;
  ...
  *errCode = cusparseGetPointerMode(c_hdl, &t_mode);
  *mode = (t_mode);
  *toplevel = true;
...
void cuSparseGetPointerMode_initialize(void)
{
}

void cuSparseGetPointerMode_terminate(void)
{
}
