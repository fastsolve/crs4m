...
void cuStreamSynchronize_api(const mxArray ** prhs, const mxArray **plhs) {
    ...
    if (mxGetNumberOfFields(prhs[0])!=3)
        mexErrMsgIdAndTxt("cuStreamSynchronize:InputStructWrongFields",
    ...
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("cuStreamSynchronize:WrongInputStruct",
    ...
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }
    ...
    cuStreamSynchronize_initialize();

    cuStreamSynchronize(&stm, errCode, toplevel);

    cuStreamSynchronize_terminate();
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs == 1) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("cuStreamSynchronize:TooManyOutputArguments","Too many output arguments for entry-point cuStreamSynchronize.");
        /* Call the API function. */
        cuStreamSynchronize_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("cuStreamSynchronize:WrongNumberOfInputs","Incorrect number of input variables for entry-point cuStreamSynchronize.");
