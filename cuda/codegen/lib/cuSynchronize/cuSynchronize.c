...
void cuSynchronize(const struct0_T *stm, int32_T *errCode, boolean_T *toplevel)
{
  boolean_T p;
  boolean_T b_p;
  int32_T k;
  ...
  if (!p) {
    ...
    emxEnsureCapacity((emxArray__common *)b_stm, i0, (int32_T)sizeof(char_T));
    k = stm->type->size[1];
    for (i0 = 0; i0 < k; i0++) {
  ...
  emxEnsureCapacity((emxArray__common *)data, i0, (int32_T)sizeof(uint8_T));
  k = stm->data->size[0];
  for (i0 = 0; i0 < k; i0++) {
...
void cuSynchronize_initialize(void)
{
}

void cuSynchronize_terminate(void)
{
}
