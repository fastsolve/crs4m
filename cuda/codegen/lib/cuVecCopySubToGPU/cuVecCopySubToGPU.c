...
void cuVecCopySubToGPU(int32_T n, const emxArray_real_T *vec, int32_T istart,
  int32_T inc, const struct0_T *cuVec, int32_T incCuVec, int32_T *errCode,
  boolean_T *toplevel)
{
  int32_T quotient;
  int32_T sizepe;
...
void cuVecCopySubToGPU_async(int32_T n, const emxArray_real_T *vec, int32_T
  istart, int32_T inc, const struct0_T *cuVec, int32_T incCuVec, const struct1_T
  *strm, int32_T *errCode, boolean_T *toplevel)
{
  int32_T k;
  int32_T sizepe;
  char * ptr;
  ...
  if ((cuVec->type != 2) && (cuVec->type != 1)) {
    ...
    k = M2C_INTDIV(cuVec->len, incCuVec);
    if (n > k) {
      b_m2c_error();
    }
  }
