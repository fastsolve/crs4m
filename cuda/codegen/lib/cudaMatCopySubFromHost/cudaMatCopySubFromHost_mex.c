/*
 * cudaMatCopySubFromHost_mex.c
 *
 * Auxiliary code for mexFunction of cudaMatCopySubFromHost
 *
 * C source code generated by m2c.
 * %#m2c options: codegenArgs=-args {m2c_int, m2c_int, m2c_mat, CudaMat}  enableInline=1
 *
 */

#ifdef MATLAB_MEX_FILE
#include "mex.h"
#else
#include "matrix.h"
#endif
/* Include the C file generated by codegen in lib mode */
#include "cudaMatCopySubFromHost.h"
#include "m2c.c"
/* Include declaration of some helper functions. */
#include "lib2mex_helper.c"

void cudaMatCopySubFromHost_api(const mxArray ** prhs, const mxArray **plhs) {

    emxArray_real_T      mat;

    struct0_T            cuMat;
    mxArray              *_sub_mx1;

    int32_T              nrows;
    int32_T              ncols;
    int32_T              *errCode;
    boolean_T            *toplevel;

    /* Marshall in function inputs */
    if (mxGetNumberOfElements(prhs[0]) && mxGetClassID(prhs[0]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputType",
            "Input argument nrows has incorrect data type. int32 is expected.");
    if (mxGetNumberOfElements(prhs[0]) != 1)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongSizeOfInputArg",
            "Argument nrows should be a scalar.");
    nrows = *(int32_T*)mxGetData(prhs[0]);
    if (mxGetNumberOfElements(prhs[1]) && mxGetClassID(prhs[1]) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputType",
            "Input argument ncols has incorrect data type. int32 is expected.");
    if (mxGetNumberOfElements(prhs[1]) != 1)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongSizeOfInputArg",
            "Argument ncols should be a scalar.");
    ncols = *(int32_T*)mxGetData(prhs[1]);
    if (mxGetNumberOfElements(prhs[2]) && mxGetClassID(prhs[2]) != mxDOUBLE_CLASS)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputType",
            "Input argument mat has incorrect data type. double is expected.");
    alias_mxArray_to_emxArray(prhs[2], (emxArray__common *)&mat, "mat", 2);

    if (!mxIsStruct(prhs[3]))
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputType",
            "Input argument cuMat has incorrect data type. struct is expected.");
    if (mxGetNumberOfFields(prhs[3])!=3)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:InputStructWrongFields",
            "Input argument cuMat has incorrect number of fields.");
    if (mxGetNumberOfElements(prhs[3]) != 1)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongSizeOfInputArg",
            "Argument cuMat must contain 1 items.");

    _sub_mx1 = mxGetField(prhs[3], 0, "data");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputStruct",
            "Input argument cuMat does not have the field data.");
    if (mxGetNumberOfElements(_sub_mx1) && mxGetClassID(_sub_mx1) != mxUINT64_CLASS)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputType",
            "Input argument cuMat.data has incorrect data type. uint64 is expected.");
    if (mxGetNumberOfElements(_sub_mx1) != 1)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongSizeOfInputArg",
            "Argument cuMat.data should be a scalar.");
    cuMat.data = *(uint64_T*)mxGetData(_sub_mx1);
    _sub_mx1 = mxGetField(prhs[3], 0, "type");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputStruct",
            "Input argument cuMat does not have the field type.");
    if (mxGetNumberOfElements(_sub_mx1) && mxGetClassID(_sub_mx1) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputType",
            "Input argument cuMat.type has incorrect data type. int32 is expected.");
    if (mxGetNumberOfElements(_sub_mx1) != 1)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongSizeOfInputArg",
            "Argument cuMat.type should be a scalar.");
    cuMat.type = *(int32_T*)mxGetData(_sub_mx1);
    _sub_mx1 = mxGetField(prhs[3], 0, "dims");
    if (_sub_mx1==NULL)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputStruct",
            "Input argument cuMat does not have the field dims.");
    if (mxGetNumberOfElements(_sub_mx1) && mxGetClassID(_sub_mx1) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongInputType",
            "Input argument cuMat.dims has incorrect data type. int32 is expected.");
    if (mxGetNumberOfElements(_sub_mx1) != 2)
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongSizeOfInputArg",
            "Argument cuMat.dims must contain 2 numbers.");
    copy_mxArray_to_array(_sub_mx1, cuMat.dims, 2);

    /* Preallocate output variables */
    {mwSize l_size[] = {1, 1};
    *(void **)&errCode = prealloc_mxArray((mxArray**)&plhs[0], mxINT32_CLASS, 2, l_size); }
    {mwSize l_size[] = {1, 1};
    *(void **)&toplevel = prealloc_mxArray((mxArray**)&plhs[1], mxLOGICAL_CLASS, 2, l_size); }

    /* Invoke the target function */
    cudaMatCopySubFromHost_initialize();

    cudaMatCopySubFromHost(nrows, ncols, &mat, &cuMat, errCode, toplevel);

    cudaMatCopySubFromHost_terminate();

    /* Marshall out function outputs */
    /* Nothing to do for plhs[0] */
    /* Nothing to do for plhs[1] */

    /* Free temporary variables */
    free_emxArray((emxArray__common*)&mat);
}

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    /* Temporary copy for mex outputs. */
    mxArray *outputs[2];
    int i;
    int nOutputs = (nlhs < 1 ? 1 : nlhs);

    if (nrhs == 4) {
        if (nlhs > 2)
            mexErrMsgIdAndTxt("cudaMatCopySubFromHost:TooManyOutputArguments","Too many output arguments for entry-point cudaMatCopySubFromHost.");
        /* Call the API function. */
        cudaMatCopySubFromHost_api(prhs, (const mxArray**)outputs);
    }
    else
        mexErrMsgIdAndTxt("cudaMatCopySubFromHost:WrongNumberOfInputs","Incorrect number of input variables for entry-point cudaMatCopySubFromHost.");

    /* Copy over outputs to the caller. */
    for (i = 0; i < nOutputs; ++i) {
        plhs[i] = outputs[i];
    }
}