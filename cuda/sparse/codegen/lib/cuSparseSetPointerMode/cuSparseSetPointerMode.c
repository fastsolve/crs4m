#include "cuSparseSetPointerMode.h"
#include "mspack.h"
#include "m2c.h"
...
static void b_m2c_error(const emxArray_char_T *varargin_3)
{
  ...
  M2C_error("cuSparse:RuntimeError",
            "cusparseSetPointerMode returned error code %s\n",
            &b_varargin_3->data[0]);
...
void cuSparseSetPointerMode(const struct0_T *hdl, int32_T mode, int32_T *errCode,
  boolean_T *toplevel)
{
  boolean_T p;
  boolean_T b_p;
  ...
  *errCode = cusparseSetPointerMode(c_hdl, mode);
  *toplevel = true;
...
void cuSparseSetPointerMode_initialize(void)
{
}

void cuSparseSetPointerMode_terminate(void)
{
}
