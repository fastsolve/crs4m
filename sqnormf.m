...
ismt = nargin>2 && MACC_get_num_threads>1;
...
if nargin>2 && ~isempty(nthreads)
    ...
    [i, s_local, iend] = MACC_begin_parfor(i, s_local, iend);
    MACC_clause_default('none');
    MACC_clause_shared( A, iend); MACC_clause_private( i); 
    MACC_clause_reduction( '+', s_local); 
    ...
    if ismt
        MACC_barrier; MACC_begin_single
        sqnrm(1) = 0;
        MACC_end_single
